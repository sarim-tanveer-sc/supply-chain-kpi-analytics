...
WITH fact_release_ot_flag_per_order_line AS (
		SELECT o.order_id, o.order_line_id, o.channel, o.dc_id,
		...
		ON o.order_id = re.order_id AND o.order_line_id = re.order_line_id
),
fact_release_if_flag_per_order_line AS (
		SELECT o.order_id, o.order_line_id, o.dc_id, o.channel,
...
fact_released_flag AS (
		SELECT ot.order_id, ot.dc_id, ot.channel,
		...
		ON ot.order_id = rif.order_id AND ot.dc_id = rif.dc_id 
		AND ot.channel = rif.channel
		GROUP BY ot.order_id, ot.dc_id, ot.channel
),
fact_order_pdt AS (
...
rel_ship_if_flag AS (
		SELECT re.order_id, re.release_if_flag, sh.shipped_if_flag
		FROM fact_released_flag AS re
		INNER JOIN fact_shipped_flag AS sh
		ON re.order_id = sh.order_id
)
SELECT res.order_id, res.release_if_flag, res.shipped_if_flag, 
			 del.delivered_ot_flag, del.delivered_if_flag,
			 MIN (res.release_if_flag, res.shipped_if_flag, 
			 del.delivered_ot_flag, del.delivered_if_flag) AS OTIF_flag
FROM rel_ship_if_flag AS res
INNER JOIN fact_delivered_flag AS del
