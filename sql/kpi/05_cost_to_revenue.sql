...
fact_ship_rev_orderline AS (
    SELECT sh.order_id, sh.order_line_id, sh.shipped_qty_orderline,
			     sh.freight_cost_orderline, o.dc_id, o.channel, o.unit_price, 
...
SELECT order_id, dc_id, channel, week_num,
			 SUM(freight_cost_orderline) AS freight_cost_order,
			 SUM(unit_price * shipped_qty_orderline) AS revenue_order,
			 ROUND((SUM(freight_cost_orderline) / SUM(unit_price * shipped_qty_orderline)) * 100.00,2) 
       AS logistics_cost_rev_pct
FROM fact_ship_rev_orderline
GROUP BY order_id, dc_id, channel, week_num;
